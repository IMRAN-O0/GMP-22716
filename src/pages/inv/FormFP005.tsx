...
export default function FormFP005() {
  const { user } = useAuth();
  const navigate = useNavigate();

  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    disposalId: generateSerialNumber("DSP", Math.floor(Math.random() * 10000)),
    itemType: "Product",
    batchOrCode: "",
    quantity: "",
    disposalReason: "",
      ...
      <div className="border-b border-slate-200 p-6 flex justify-between items-center bg-slate-50">
        <div>
          ...
          <p className="text-sm font-semibold text-slate-500 mt-1">
            تتبع عمليات إتلاف المواد والتوالف
          </p>
        </div>
        <div className="text-left">
          <h2 className="text-xl font-bold text-slate-800 mb-1">
            إتلاف المنتجات/المواد
          </h2>
          <p className="text-sm font-mono text-slate-500">نموذج: F-FP-005</p>
        </div>
      </div>
      <form className="p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">
              نوع المادة المتلفة <span className="text-red-500">*</span>
            </label>
            <select
              required
          ...
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">
              رقم الدفعة / الكود <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              required
              value={formData.batchOrCode}
              onChange={(e) =>
                setFormData({ ...formData, batchOrCode: e.target.value })
              }
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-sky-400 text-sm py-2"
            />
          </div>
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">
              الكمية المتلفة <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              required
          ...
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">
              تاريخ الإتلاف <span className="text-red-500">*</span>
            </label>
            <input
              type="date"
              required
          ...
          <div className="md:col-span-2">
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">
              سبب الإتلاف (تالف، منتهي الصلاحية، الخ...)
            </label>
            <textarea
              rows={2}
              required
