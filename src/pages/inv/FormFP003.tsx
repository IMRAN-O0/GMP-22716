  ...
  const handleSubmit = async (e: React.FormEvent, status: any) => {
    ...
    try {
      ...
      const res = await fetch(editIdPatch ? `/api/forms/record/${editIdPatch}` : "/api/forms", {
        ...
        body: JSON.stringify({ recordId: formData.shipmentId, formId: "F-FP-003", department: "INV", creatorId: user?.id, status, data: formData }),
      });
    ...
    <div className="max-w-4xl mx-auto bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
      <div className="border-b border-slate-200 p-6 flex justify-between items-center bg-slate-50">
        <div>
          ...
          <p className="text-sm font-semibold text-slate-500 mt-1">تتبع شحنات المنتجات للعملاء</p>
        </div>
        <div className="text-left">
          <h2 className="text-xl font-bold text-slate-800 mb-1">شحن المنتجات</h2>
      ...
      <form className="p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div>
            <SearchField label="العميل / الوجهة" required value={formData.customerName}
          ...
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">العنوان التفصيلي <span className="text-red-500">*</span></label>
            <input type="text" required value={formData.destinationAddress} onChange={e => setFormData({ ...formData, destinationAddress: e.target.value })}
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-sky-400 text-sm py-2" />
          </div>

          {/* Batch + Product */}
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">رقم الدفعة المشحونة <span className="text-red-500">*</span></label>
          ...
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">الكمية المشحونة <span className="text-red-500">*</span></label>
            <input type="number" required min="0.01" step="0.01" value={formData.shippedQuantity} onChange={e => setFormData({ ...formData, shippedQuantity: e.target.value })}
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-sky-400 text-sm py-2" />
          </div>
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">تاريخ الشحن <span className="text-red-500">*</span></label>
            <input type="date" required value={formData.shipmentDate} onChange={e => setFormData({ ...formData, shipmentDate: e.target.value })}
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-sky-400 text-sm py-2" />
          </div>
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">اسم الناقل / السائق <span className="text-red-500">*</span></label>
            <input type="text" required value={formData.transporterName} onChange={e => setFormData({ ...formData, transporterName: e.target.value })}
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-sky-400 text-sm py-2" />
          </div>
          <div>
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">رقم المركبة</label>
            <input type="text" value={formData.vehiclePlate} onChange={e => setFormData({ ...formData, vehiclePlate: e.target.value })}
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-sky-400 text-sm py-2" />
          </div>
          <div className="md:col-span-2">
            <label className="block text-[13px] font-semibold text-slate-600 mb-1">ملاحظات والتوجيه</label>
        ...
        {formData.productCode && (
          <div className="mb-6 p-3 bg-rose-50 border border-rose-200 rounded-lg text-sm">
            <span className="font-bold text-rose-700">سيتم خصم </span>
            <span className="font-bold text-rose-900">{formData.shippedQuantity || "—"} </span>
            <span className="font-bold text-rose-700">وحدة من المنتج </span>
            <span className="font-mono font-bold text-rose-900">{formData.productCode}</span>
            {formData.productName && <span className="text-rose-700"> ({formData.productName})</span>}
            <span className="text-rose-700"> عند الاعتماد</span>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-slate-200">
          ...
          {user?.level <= 2 ? (
            <button type="button" disabled={loading} onClick={e => handleSubmit(e, "approved")}
              ...
              <Truck className="w-4 h-4" /> حفظ واعتماد
            </button>
          ) : (
