...
export default function FormHRT008() {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent, status: any = "approved") => {
    ...
    try {
      ...
      const res = await fetch(fetchUrl, {
        ...
        body: JSON.stringify({
          recordId: formData.warningId || nextSequentialId("HRT-WRN", []),
          formId: "F-HRT-008",
          department: "HRT",
          creatorId: user?.id,
          status,
    ...
    if (editId) {
      ...
      fetch("/api/forms/dept/HRT", { headers: getAuthHeaders() })
        ...
        .then((data) => {
          ...
          const ids = rows.filter((f: any) => f.form_id === "F-HRT-008").map((f: any) => f.record_id);
          setFormData((prev) => ({ ...prev, warningId: nextSequentialId("HRT-WRN", ids) }));
        })
        .catch(() => {});
    }
  }, []);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6 bg-white p-4 rounded-xl border border-sky-200 shadow-sm border-r-4 border-r-sky-500">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">لفت نظر / إنذار كتابي</h1>
          <p className="text-slate-500">النموذج: F-HRT-008 | الموارد البشرية والتدريب</p>
        </div>
      </div>
        ...
        <div className="p-6 space-y-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">رقم الإشعار (تلقائي)</label>
                <input type="text" readOnly className="w-full px-4 py-2 bg-slate-100 border border-slate-200 rounded-lg text-slate-500" value={formData.warningId} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">تاريخ الإشعار <span className="text-red-500">*</span></label>
                <input type="date" required className={inputCls} value={formData.noticeDate} onChange={(e) => setFormData({ ...formData, noticeDate: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">نوع الإجراء</label>
                <select className={inputCls} value={formData.warningType} onChange={(e) => setFormData({ ...formData, warningType: e.target.value })}>
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800 mb-4 border-b border-slate-200 pb-2">تفاصيل المخالفة</h3>
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">تاريخ المخالفة</label>
                <input type="date" className={inputCls} value={formData.violationDate} onChange={(e) => setFormData({ ...formData, violationDate: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">صادر عن</label>
                <input type="text" className={inputCls} value={formData.issuedBy} onChange={(e) => setFormData({ ...formData, issuedBy: e.target.value })} />
              </div>
            </div>
            <div className="mb-6">
              <label className="block text-sm font-semibold text-slate-700 mb-2">وصف المخالفة / التقصير <span className="text-red-500">*</span></label>
              <textarea rows={3} required className={inputCls} value={formData.violationDescription} onChange={(e) => setFormData({ ...formData, violationDescription: e.target.value })} />
            </div>
            <div className="mb-6">
              <label className="block text-sm font-semibold text-slate-700 mb-2">الإجراء المطلوب</label>
              <textarea rows={2} className={inputCls} value={formData.actionRequired} onChange={(e) => setFormData({ ...formData, actionRequired: e.target.value })} />
            </div>
            <div>
