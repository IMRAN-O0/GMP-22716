...
export default function FormHRT012() {
  const { user } = useAuth();
  ...
  const [formData, setFormData] = useState({
    ...
    reasonForLeaving: "",
    feedback: [
      { topic: "بيئة العمل وثقافة الشركة", rating: "", comment: "" },
      { topic: "العلاقة مع الإدارة المباشرة", rating: "", comment: "" },
      { topic: "فرص التطور والتدريب", rating: "", comment: "" },
      { topic: "الرواتب والمزايا", rating: "", comment: "" },
      { topic: "إجراءات العمل والأنظمة", rating: "", comment: "" },
    ] as { topic: string; rating: string; comment: string }[],
    suggestions: "",
    wouldRecommend: "",
    notes: "",
  ...
  const updateFeedback = (i: number, key: "rating" | "comment", val: string) => {
    setFormData((prev) => ({
      ...prev,
      feedback: prev.feedback.map((f, idx) => (idx === i ? { ...f, [key]: val } : f)),
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent, status: any = "approved") => {
    ...
    try {
      ...
      const res = await fetch(fetchUrl, {
        ...
        body: JSON.stringify({
          recordId: formData.exitInterviewId || nextSequentialId("HRT-EXT", []),
          formId: "F-HRT-012",
          department: "HRT",
          creatorId: user?.id,
          status,
    ...
    if (editId) {
      ...
      fetch("/api/forms/dept/HRT", { headers: getAuthHeaders() })
        ...
        .then((data) => {
          ...
          const ids = rows.filter((f: any) => f.form_id === "F-HRT-012").map((f: any) => f.record_id);
          setFormData((prev) => ({ ...prev, exitInterviewId: nextSequentialId("HRT-EXT", ids) }));
        })
        .catch(() => {});
    }
  }, []);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6 bg-white p-4 rounded-xl border border-sky-200 shadow-sm border-r-4 border-r-sky-500">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">مقابلة خروج (Exit Interview)</h1>
          <p className="text-slate-500">النموذج: F-HRT-012 | الموارد البشرية والتدريب</p>
        </div>
      </div>
        ...
        <div className="p-6 space-y-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">رقم المقابلة (تلقائي)</label>
                <input type="text" readOnly className="w-full px-4 py-2 bg-slate-100 border border-slate-200 rounded-lg text-slate-500" value={formData.exitInterviewId} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">تاريخ المغادرة <span className="text-red-500">*</span></label>
                <input type="date" required className={inputCls} value={formData.exitDate} onChange={(e) => setFormData({ ...formData, exitDate: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">سبب ترك العمل</label>
                <select className={inputCls} value={formData.reasonForLeaving} onChange={(e) => setFormData({ ...formData, reasonForLeaving: e.target.value })}>
                  <option value="">— اختر —</option>
                  <option value="فرصة أفضل">فرصة أفضل</option>
                  <option value="أسباب شخصية">أسباب شخصية</option>
                  <option value="عدم الرضا الوظيفي">عدم الرضا الوظيفي</option>
                  <option value="التقاعد">التقاعد</option>
                  <option value="أخرى">أخرى</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">هل توصي بالعمل في الشركة</label>
                <select className={inputCls} value={formData.wouldRecommend} onChange={(e) => setFormData({ ...formData, wouldRecommend: e.target.value })}>
                  <option value="">— اختر —</option>
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800 mb-4 border-b border-slate-200 pb-2">تقييم تجربة العمل (من 1 إلى 5)</h3>
            <table className="w-full text-right text-sm border-collapse border border-slate-200 rounded-lg overflow-hidden">
              <thead className="bg-slate-100 text-slate-600">
                <tr>
                  <th className="p-3 border-b border-slate-200">المحور</th>
                  <th className="p-3 border-b border-slate-200 w-28">التقييم</th>
                  <th className="p-3 border-b border-slate-200">التعليق</th>
                </tr>
              </thead>
              <tbody>
                {formData.feedback.map((f, i) => (
                  <tr key={i} className="border-b border-slate-100 last:border-0">
                    <td className="p-3 font-semibold text-slate-800">{f.topic}</td>
                    <td className="p-3">
                      <input type="number" min={1} max={5} className="w-20 px-2 py-1.5 border border-slate-300 rounded text-sm" value={f.rating} onChange={(e) => updateFeedback(i, "rating", e.target.value)} />
                    </td>
                    <td className="p-3">
                      <input type="text" className="w-full px-2 py-1.5 border border-slate-300 rounded text-sm" value={f.comment} onChange={(e) => updateFeedback(i, "comment", e.target.value)} />
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div>
            <div className="mb-6">
              <label className="block text-sm font-semibold text-slate-700 mb-2">اقتراحات للتحسين</label>
              <textarea rows={2} className={inputCls} value={formData.suggestions} onChange={(e) => setFormData({ ...formData, suggestions: e.target.value })} />
            </div>
            <div>
