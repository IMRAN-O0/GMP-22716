...
export default function FormHRT011() {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent, status: any = "approved") => {
    ...
    try {
      ...
      const res = await fetch(fetchUrl, {
        ...
        body: JSON.stringify({
          recordId: formData.resignationId || nextSequentialId("HRT-RSG", []),
          formId: "F-HRT-011",
          department: "HRT",
          creatorId: user?.id,
          status,
    ...
    if (editId) {
      ...
      fetch("/api/forms/dept/HRT", { headers: getAuthHeaders() })
        ...
        .then((data) => {
          ...
          const ids = rows.filter((f: any) => f.form_id === "F-HRT-011").map((f: any) => f.record_id);
          setFormData((prev) => ({ ...prev, resignationId: nextSequentialId("HRT-RSG", ids) }));
        })
        .catch(() => {});
    }
  }, []);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6 bg-white p-4 rounded-xl border border-sky-200 shadow-sm border-r-4 border-r-sky-500">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">قبول استقالة / إنهاء خدمات</h1>
          <p className="text-slate-500">النموذج: F-HRT-011 | الموارد البشرية والتدريب</p>
        </div>
      </div>
        ...
        <div className="p-6 space-y-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">رقم الطلب (تلقائي)</label>
                <input type="text" readOnly className="w-full px-4 py-2 bg-slate-100 border border-slate-200 rounded-lg text-slate-500" value={formData.resignationId} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">تاريخ تقديم الطلب</label>
                <input type="date" className={inputCls} value={formData.submitDate} onChange={(e) => setFormData({ ...formData, submitDate: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select className={inputCls} value={formData.requestType} onChange={(e) => setFormData({ ...formData, requestType: e.target.value })}>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">آخر يوم عمل <span className="text-red-500">*</span></label>
                <input type="date" required className={inputCls} value={formData.lastWorkingDay} onChange={(e) => setFormData({ ...formData, lastWorkingDay: e.target.value })} />
              </div>
            </div>
          </div>

          <div>
            <h3 className="text-lg font-bold text-slate-800 mb-4 border-b border-slate-200 pb-2">التفاصيل</h3>
            <div className="mb-6">
              <label className="block text-sm font-semibold text-slate-700 mb-2">السبب</label>
              <textarea rows={3} className={inputCls} value={formData.reason} onChange={(e) => setFormData({ ...formData, reason: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">القرار</label>
