...
// INV: Recalculate all balances from approved form records (admin only)
router.post("/materials/recalculate-balances", requireAuth, (req: any, res) => {
  const user = req.user;
  ...
  db.all("SELECT * FROM forms_records WHERE status = 'approved' ORDER BY created_at ASC", [], (err, rows: any[]) => {
    ...
    // Build balance map: code -> number
    const balances: Record<string, number> = {};

    // Step 1: seed from material definition forms (RM-001, MAT)
    rows.filter(r => r.form_id === "F-INV-RM-001" || r.form_id === "F-INV-MAT").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
        if (d.code) balances[d.code] = parseFloat(d.balance) || 0;
      } catch { /* skip */ }
    });

    // Step 2: PIN - add received quantities
    rows.filter(r => r.form_id === "F-INV-PIN-001").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
    ...
    // Step 3: RMT - add or subtract (skip Receive linked to PIN)
    rows.filter(r => r.form_id === "F-INV-RMT-001").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
        if (d.transactionType === "Receive" && d.referenceDocument && /^PIN-/i.test(String(d.referenceDocument))) return;
        if (Array.isArray(d.items)) {
          const multiplier = d.transactionType === "Receive" ? 1 : -1;
          d.items.forEach((item: any) => {
            const qty = parseFloat(item.quantity) || 0;
            if (item.materialCode && qty > 0) {
              balances[item.materialCode] = Math.max(0, (balances[item.materialCode] || 0) + qty * multiplier);
            }
          });
        }
      } catch { /* skip */ }
    ...
    // Step 4: PRD-002 - deduct raw materials used in production
    const prd001Map: Record<string, any> = {};
    rows.filter(r => r.form_id === "F-PRD-001").forEach(r => {
      try { prd001Map[r.record_id] = JSON.parse(r.data_json || "{}"); } catch { /* skip */ }
    });
    rows.filter(r => r.form_id === "F-PRD-002").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
        const prd1 = prd001Map[d.productionOrderNo];
        const rawMats = d.rawMaterials || (prd1 ? prd1.rawMaterials : []) || [];
        rawMats.forEach((mat: any) => {
          const qty = parseFloat(mat.quantity) || 0;
          if (mat.materialCode && qty > 0) {
    ...
    // Step 5: FP-002 - add stored finished product
    const prd001ByBatch: Record<string, string> = {};
    rows.filter(r => r.form_id === "F-PRD-001").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
    ...
    rows.filter(r => r.form_id === "F-FP-002").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
        const code = d.productCode || (d.batchNumber ? prd001ByBatch[d.batchNumber] : undefined);
        const qty = parseFloat(d.quantityStored) || 0;
        if (code && qty > 0) balances[code] = (balances[code] || 0) + qty;
      } catch { /* skip */ }
    });

    // Step 6: FP-003 - deduct shipped quantity
    rows.filter(r => r.form_id === "F-FP-003").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
        const code = d.productCode || (d.batchNumber ? prd001ByBatch[d.batchNumber] : undefined);
        const qty = parseFloat(d.shippedQuantity) || 0;
        if (code && qty > 0) balances[code] = Math.max(0, (balances[code] || 0) - qty);
      } catch { /* skip */ }
    });

    // Step 7: FP-004 - add returned quantity
    rows.filter(r => r.form_id === "F-FP-004").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
        const code = d.productCode || (d.batchNumber ? prd001ByBatch[d.batchNumber] : undefined);
    ...
    // Step 8: FP-005 - deduct disposed quantity
    rows.filter(r => r.form_id === "F-FP-005").forEach(r => {
      try {
        const d = JSON.parse(r.data_json || "{}");
        const qty = parseFloat(d.quantity) || 0;
        if (d.batchOrCode && qty > 0) balances[d.batchOrCode] = Math.max(0, (balances[d.batchOrCode] || 0) - qty);
      } catch { /* skip */ }
    });

    // Apply computed balances to materials table
    ...
    db.serialize(() => {
      ...
      db.run("COMMIT", (commitErr) => {
        if (commitErr) {
          db.run("ROLLBACK");
          return res.status(500).json({ error: "فشل تطبيق التحديثات" });
        }
