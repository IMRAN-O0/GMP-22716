...
## 🖧 وضع السيرفر / العميل (ربط الأجهزة بقاعدة بيانات واحدة)

نفس الـ.exe يعمل بوضعين، ويُسأل عن ذلك **مرة واحدة فقط** عند أول تشغيل عبر شاشة إعداد:

| الوضع | الوصف |
|---|---|
| 🖥️ **جهاز رئيسي (سيرفر)** | يستضيف قاعدة البيانات. ثبّته على **جهاز واحد** فقط. |
| 🔗 **جهاز عميل** | لا يخزّن بيانات؛ يتصل بالجهاز الرئيسي ويشاركه نفس قاعدة البيانات. |

**خطوات الربط:**
1. على الجهاز الرئيسي: شغّل البرنامج واختر **«جهاز رئيسي (سيرفر)»**. اعرف عنوانه بـ `ipconfig` (مثل `192.168.1.10`).
2. على كل جهاز آخر: شغّل البرنامج، اختر **«جهاز عميل»**، اكتب عنوان الجهاز الرئيسي، واضغط **«اختبار الاتصال»** ثم **«حفظ وبدء التشغيل»**.
3. من الآن فصاعداً يفتح كل جهاز كتطبيق سطح مكتب عادي — بدون رابط وبدون متصفح.

> 💡 لتغيير عنوان السيرفر لاحقاً: اضغط **Ctrl+Alt+S** داخل البرنامج لإعادة فتح شاشة الإعداد.
>
> ملاحظات: يجب أن يكون الجهاز الرئيسي مشغّلاً، ويُفضّل تعيين IP ثابت له. الإعداد محفوظ في
> `gmp-config.json` داخل مجلد بيانات التطبيق.
...
## 🔐 الوصول البعيد الآمن (خارج المصنع)

للوصول من خارج الشبكة المحلية **دون تعريض النظام للإنترنت ودون نقل بيانات غير مشفّرة**،
استخدم شبكة VPN مشفّرة (Tailscale أو WireGuard). فحص CORS يقبل نطاق Tailscale
(`100.64.0.0/10`) تلقائياً.

📖 الدليل الكامل خطوة بخطوة: [`docs/REMOTE-ACCESS.md`](docs/REMOTE-ACCESS.md)

> ⚠️ لا تفتح المنفذ 3009 للإنترنت مباشرةً. اجعل الوصول البعيد عبر VPN فقط.
